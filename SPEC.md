...
Standalone Node.js microservice. Runs persistently alongside the Python backend.

### What it does
- Holds the WhatsApp Web session (QR scan once, session persisted to disk)
- Listens to `messages.upsert` — fires for every message on the advisor's number
- Filters: only forwards messages where the sender/recipient is a tracked client
- POSTs filtered messages to Python backend
...
sock.ev.on('messages.upsert', async ({ messages, type }) => {
  ...
  for (const msg of messages) {
    ...
    // check if this number is a tracked client
    const { exists } = await fetch(
      `http://backend/clients/exists?number=${phone}`
    ).then(r => r.json())

    if (!exists) return   // discard — not a client

    await fetch('http://backend/internal/messages', {
      method: 'POST',
      body: JSON.stringify({
        phone,
...
## 8. Python Backend — API Endpoints
